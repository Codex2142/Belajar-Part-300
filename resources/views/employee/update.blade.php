        ...
        <div class="container-md">
            ...
            <form action="{{route('employee.update', $employee->id)}}" method="POST">
                ...
                <div class="mb-3">
                  ...
                  <input type="text" class="form-control" id="firstname" name="firstname" value="{{old('fistname', $employee->firstname)}}">
                </div>
                @error('firstname')
                ...
                <div class="mb-3">
                  ...
                  <select class="form-select" aria-label="Default select example" id="dept_id" name="dept_id">
                    @foreach ($department as $d )
                        <option value="{{$d->id}}" {{$employee->dept_id == $d->id ? 'selected': ''}}>{{$d->name}}</option>
                    @endforeach
                  </select>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" aria-label="Default select example" id="status" name="status">
                        <option value="cont" {{$employee->status == 'cont' ? 'selected': ''}}>Contract</option>
                        <option value="emp" {{$employee->status == 'emp' ? 'selected': ''}}>Employee</option>
                        <option value="not_act" {{$employee->status == 'not_act' ? 'selected': ''}}>Not Active</option>
                    </select>
                </div>
